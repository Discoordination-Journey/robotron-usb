...
static void usbGenericSetInterrupt(uchar *data, uchar len, usbTxStatus_t *txStatus)
{
    ...
    if(txStatus->len & 0x10 )txStatus->buffer[0] ^= USBPID_DATA0 ^ USBPID_DATA1; // toggle token
	else txStatus->len = USBPID_NAK; // avoid sending outdated (overwritten) interrupt data
	...
	// len must be given including sync byte
    txStatus->len = len + 4;    
